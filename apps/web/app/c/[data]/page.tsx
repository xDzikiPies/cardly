...
import { decodeCardFromUrlParam } from "@/lib/cardEncoding";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const card = decodeCardFromUrlParam(data);
...
export default async function PublicCardPage({ params }: PageProps) {
    ...
    return (
      <main className={styles.main}>
        <div className={styles.notFound}>
          ...
          <p className={styles.notFoundBody}>Link może być niepoprawny albo uszkodzony.</p>
        </div>
      </main>
    );
  }
  ...
  const vcardHref = vCardDataUri(card);
  ...
  return (
    <main className={styles.main}>
      <div className={styles.wrap}>
        ...
        <InteractiveBusinessCard card={card} background={background} />
